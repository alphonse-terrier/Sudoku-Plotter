...
\begin{figure}[!h]
 \center
 \includegraphics[scale=0.4]{../pictures/schema_stepper_motor.png}
 \caption{Schéma électrique réalisé avec \emph{Fritzing}}
 \label{schema_stepper}
\end{figure}

\paragraph{} Les moteurs que nous avons utilisés sont constitués de quatre bobines reliés deux à deux, soit quatre fils, comme le montre le schéma suivant, ainsi les bobines A et B seront désignées comme la bobine 1, C et D comme la bobine 2. Ces bobines, lorsqu'elles sont parcourues par un courant i, génère un champ magnétique. Ce champ magnétique a pour conséquence d'entrainer la rotation de la partie mobile du moteur : le stator, représenté par le cercle centrale N/S sur la figure \ref{schema_stepper}. Celui-ci est composé d'un aimant qui a tendance a s'aligner sur le champ magnétique de la bobine. 
...
\begin{tabularx}{0.5\linewidth}{|c|*{5}{>{\centering \arraybackslash}X|}}\hline
\multirow{2}*{Séquence} & \multicolumn{2}{c|}{Bobine 1}  & \multicolumn{2}{c|}{Bobine 2} \\
\cline{2-5} & \centering A & \centering B & \centering C & D \\ 
\hline  
1 & 1 & 0 & - & - \\
\hline
2 & - & - & 0 & 1 \\
...
\paragraph{} On distingue deux types de fonctionnement pour ces moteurs bipolaires, un fonctionnement à demi-pas et un fonctionnement à pas complet. Nous avons retenu ce dernier système car il permettait d'obtenir un couple et une vitesse constants lors de la rotation des moteurs. \`{A} chaque séquence, une seule bobine suffirait à mettre en mouvement le moteur, l'autre est tout de même alimentée afin d'obtenir un couple maximal. 

Une autre différence entre l'alimentation d'une ou deux bobines réside également dans la position de l'axe des deux moteurs, la position de celui-ci est décalé d'un demi-pas. Le fonctionnement en demi-pas se base sur la combinaison de ces deux méthodes, ce qui entraîne un couple variable.
