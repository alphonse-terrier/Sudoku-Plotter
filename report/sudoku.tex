...
Les moteurs que nous avons utilisés sont constitués de deux bobines, soit quatre fils. On distingue deux types de fonctionnement pour ces moteurs bipolaires, un fonctionnement à demi-pas et un fonctionnement à pas complet. Nous avons retenu ce dernier système car il permettait d'obtenir un couple et une vitesse constants lors de la rotation des moteurs. \`{A} chaque séquence, une seule bobine suffirait à mettre en mouvement le moteur, l'autre est tout de même alimentée afin d'obtenir un couple maximal. 

\begin{figure}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
Séquence & \multicolumn{2}{c|}{Bobine 1}  & \multicolumn{2}{c|}{Bobine 2} \\
\hline
1 & 1 & 0 & - & - \\
\hline
2 & - & - & 0 & 1 \\
...
\hline
\end{tabular}
\caption{Séquences d'alimentation des bobines avec un couple minimal}
\end{center}
\end{figure}
...
\hline
\end{tabular}
\caption{Séquences d'alimentation des bobines avec un couple maximal}
\end{center}
\end{figure}
Une autre différence entre l'alimentation que d'une ou deux bobines réside également dans la position de l'axe des deux moteurs, la position de celui-ci est décalé d'un demi-pas. Le fonctionnement en demi-pas se base sur la combinaison de ces deux méthodes, ce qui entraîne un couple variable.
